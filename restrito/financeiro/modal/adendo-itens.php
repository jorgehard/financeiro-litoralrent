	...
	if(isset($ac)){
		if($ac=='add'){
			try 
			{
				...
				//$stm->execute(array($id_contrato, $equipamentoInput, $vlrInput, $data_retiradaInput, 0, $id_adendo));
				$stm = $con->query("INSERT INTO contrato_itens (id_contrato, equipamento, vlr, data_retirada, tipo, obs, adendo)
				
				SELECT '$id_contrato', '$equipamentoInput', '$vlrInput', '$data_retiradaInput', '0', '$obsInput', '$id_adendo' FROM DUAL WHERE NOT EXISTS
				
				(SELECT equipamento FROM contrato_itens WHERE id_contrato = '$id_contrato' AND equipamento = '$equipamentoInput' AND tipo = '0' AND adendo = '$id_adendo')");
				$inser = $con->query("UPDATE notas_equipamentos SET controle = '1' WHERE id = '$equipamentoInput'");
				
				echo '<script>ldy("financeiro/modal/listar-adendo.php?id_adendo='.$id_adendo.'&id_contrato='.$id_contrato.'","#listar'.$id_adendo.'") </script>';
				
				echo '<script>ldy("../functions/functions-load.php?atu=tipo_contrato","#itens_tipo'.$id_adendo.'") </script>';
			}
...
?>
<script>
$('#autoComplete<?= $x['id'] ?>').selectToAutocomplete();
</script>
	<?php
		if($x['tipo'] == '0') {
		...
		echo '<a href="#" onclick=\'$(".modal-body").load("financeiro/modal/editar-adendo.php?id_contrato='.$id_contrato.'&id_adendo='.$x['id'].'")\' style="margin:10px"  data-toggle="modal" data-target="#myModal2"  class="pull-right btn btn-info btn-sm"><span class="glyphicon glyphicon-edit"></span> Editar</a>';
		...
		<div class="row-fluid" style="background:#FFF; margin-bottom:20px;">
			...
			<div class="box box-widget box-fix-layout" style=" background:#f0fbf9">
				<div class="container-fluid" style="padding:5px">
					<form action="javascript:void(0)" onSubmit="post(this,'financeiro/modal/adendo-itens.php?ac=add&id_adendo=<?= $x['id'] ?>&id_contrato=<?= $id_contrato ?>','#listar<?= $x['id'] ?>')">
						<div class="box-body">
							<div class="col-md-2 col-xs-12">
								<div class="form-group">
									<label><small>Data:</small></label>
									<input type="date" name="data_retiradaInput" min="2018-01-01" value="<?= $todayTotal ?>" class="form-control input-sm" required />
								</div>
							</div>
			...
			<script>ldy("financeiro/modal/listar-adendo.php?id_adendo=<?= $x['id'] ?>&id_contrato=<?= $id_contrato ?>","#listar<?= $x['id'] ?>")</script>
