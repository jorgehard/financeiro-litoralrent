		...
		foreach($ca as $cat) { @$catu .= $cat.','; } $catu = substr($catu,0,-1);
			echo '<div class="box box-widget">
				...
				while($b = $stm->fetch())
				{
					...
					echo '<td width="5px" style="text-align:center"><a href="#" Onclick=\'$(".modal-body").load("financeiro/editar-item.php?id='.$b['id'].'")\' data-toggle="modal" data-target="#myModal" class="btn btn-success btn-xs" style="margin:0px; font-weight:bold;"><span class="glyphicon glyphicon-pencil"></span></a></td>';
...
<section class="content">
	...
	<form action="javascript:void(0)" id="form1" class="hidden-print">
		<div class="well well-sm" style="padding:10px 10px 5px 10px;">
			...
			<label><br/>
				<input type="submit" value="Pesquisar" style="width:150px; margin-left:10px;" onClick="post('#form1','financeiro/consulta-itens.php?ac=consulta','.retorno')" class="btn btn-success btn-sm">
			</label>
		</div>
	</form>
